...
import com.megahed.pdfview.enms.ScrollDir;
import com.megahed.pdfview.enms.SnapEdge;
import com.megahed.pdfview.enms.State;
import com.megahed.pdfview.exception.PageRenderingException;
import com.megahed.pdfview.link.DefaultLinkHandler;
import com.megahed.pdfview.link.LinkHandler;
import com.megahed.pdfview.listener.Callbacks;
...
import com.megahed.pdfview.util.Constants;
import com.megahed.pdfview.util.MathUtils;
 ...
 * #( String, int[])}. In this particular case, a userPage of 5 can refer to a
 * documentPage of 17.
 */
public class PDFView extends RelativeLayout {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (bg == null) {
      canvas.drawColor(nightMode ? Color.GRAY : Color.GRAY);
    } else {
      bg.draw(canvas);
    }
  ...
  private void setLandscapeOrientation(boolean landscapeOrientation) {
    this.isLandscapeOrientation = landscapeOrientation;
  }

  private void setDualPageMode(boolean dualPageMode) {
    this.dualPageMode = dualPageMode;
  }

  private void setHasCover(boolean hasCover) {
  ...
  private void enableAnnotationRendering(boolean annotationRendering) {
  ...
  private void enableRenderDuringScale(boolean renderDuringScale) {
  ...
  private void enableAntialiasing(boolean enableAntialiasing) {
  ...
  private void setPageFling(boolean pageFling) {
  ...
  private void setPageSnap(boolean pageSnap) {
  ...
  public PDFView setConfigurator( Configurator configurator){
    this.configurator=configurator;
    return this;
  }
  public PDFView linkHandler(LinkHandler linkHandler) {
  ...
  public void load() {
    if (!hasSize) {
      waitingDocumentConfigurator = configurator;
    ...
    this.callbacks.setOnLoadComplete(configurator.getOnLoadCompleteListener());
    this.callbacks.setOnError(configurator.getOnErrorListener());
    this.callbacks.setOnDraw(configurator.getOnDrawListener());
    this.callbacks.setOnDrawAll(configurator.getOnDrawAllListener());
    this.callbacks.setOnPageChange(configurator.getOnPageChangeListener());
    this.callbacks.setOnZoomChange(configurator.getOnZoomChangeListener());
    this.callbacks.setOnScroll(configurator.getOnScrollListener());
    this.callbacks.setOnPageScroll(configurator.getOnPageScrollListener());
    this.callbacks.setOnRender(configurator.getOnRenderListener());
    this.callbacks.setOnTap(configurator.getOnTapListener());
    this.callbacks.setOnLongPress(configurator.getOnLongPressListener());
    this.callbacks.setOnPageError(configurator.getOnPageErrorListener());
    this.callbacks.setLinkHandler(linkHandler);
    this.setSwipeEnabled(configurator.isEnableSwipe());
    this.setNightMode(configurator.isNightMode());
    this.enableDoubletap(configurator.isEnableDoubletap());
    this.setDefaultPage(configurator.getDefaultPage());
    this.setLandscapeOrientation(configurator.isLandscapeOrientation());
    this.setDualPageMode(configurator.isDualPageMode());
    this.setHasCover(configurator.isHasCover());
    this.setBackGroundColor(configurator.getBackgroundColor());
    this.setSwipeVertical(!configurator.isSwipeHorizontal());
    this.enableAnnotationRendering(configurator.isAnnotationRendering());
    this.setScrollHandle(configurator.getScrollHandle());
    this.enableAntialiasing(configurator.isAntialiasing());
    this.setSpacing(configurator.getSpacing());
    this.setAutoSpacing(configurator.isAutoSpacing());
    //new
    this.setAutoReleasingWhenDetachedFromWindow(configurator.isAutoReleasingWhenDetachedFromWindow());
    this.setPageFitPolicy(configurator.getPageFitPolicy());
    this.setFitEachPage(configurator.isFitEachPage());
    this.setPageSnap(configurator.isPageSnap());
    this.setPageFling(configurator.isPageFling());

    if (configurator.getPageNumbers() != null) {
      this.load(configurator.getDocumentSource(), configurator.getPassword(), configurator.getPageNumbers());
    } else {
      this.load(configurator.getDocumentSource(), configurator.getPassword());
    }
  }
